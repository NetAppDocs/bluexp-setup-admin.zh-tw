...
將資源與資料夾關聯並不會自動使資料夾中的所有項目都可以存取該資源。資料夾或專案管理員決定哪些項目可以存取該資源。做出決定之後、管理員就可以將資源與正確的專案建立關聯。
...
組織:: 當您在組織層級授予成員存取角色時，組織中的所有資料夾、專案和資源都會繼承這些角色。這表示成員對組織中的一切都有權限。
