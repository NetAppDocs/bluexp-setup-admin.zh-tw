...
[role="lead"]
對於AWS商業區域而言、最好直接從Cloud Manager建立Connector、但您也可以從AWS Marketplace（如果您願意）啟動Connector。對於AWS政府區域、您無法從Cloud Manager的SaaS網站將Connector部署到政府區域、因此下一個最佳選擇是從AWS Marketplace部署Connector。
...
您可以直接從AWS Marketplace產品項目for Cloud Manager、在AWS商業區域啟動Connector執行個體。
...
. 前往 https://aws.amazon.com/marketplace/pp/B018REK8QG["AWS Marketplace 上的 Cloud Manager 頁面"^] 若要從AMI部署Cloud Manager：
...
+
** *名稱和標記*：輸入執行個體的名稱和標記。
** *應用程式與作業系統映像*：跳過本節。Cloud Manager AMI已選取。
...
** *設定儲存設備*：保留預設的儲存選項。
** *進階詳細資料*：在「* IAM intance profile *」下、選擇您在步驟1中建立的IAM角色。
...
. 開啟網頁瀏覽器並前往 https://cloudmanager.netapp.com[] 開始使用Connector with Cloud Manager。
...
若要在AWS政府區域中部署Connector、您需要前往EC2服務、並從AWS Marketplace選取Cloud Manager產品。
...
. 前往AWS Marketplace的Cloud Manager產品。
+
...
.. 選擇* AWS Marketplace *。
.. 搜尋Cloud Manager並選取產品項目。
+
image:screenshot-gov-cloud-mktp.png["在AWS Marketplace中搜尋Cloud Manager產品後、會顯示快照"]
...
只要您想使用Cloud Manager、請開啟網頁瀏覽器並連線至Connector執行個體的IP位址： http://_ipaddress_:80[]
