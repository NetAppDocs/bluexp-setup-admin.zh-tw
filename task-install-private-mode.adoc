...
如果您想在中使用 BlueXP https://["AWS 商業雲端服務（ C2S ）環境"^] 接著、您應該遵循個別指示、開始使用該環境。 https://["瞭解如何在 AWS C2S 環境中開始使用 Cloud Volumes ONTAP"^]
...
----
. 從下載Connector軟體 https://["NetApp 支援網站"^]
+
...
.步驟
. 開啟網頁瀏覽器並輸入 https://[] 其中 _ipaddress_ 是您安裝 Connector 的 Linux 主機的 IP 位址。
+
您應該會看到下列畫面。
