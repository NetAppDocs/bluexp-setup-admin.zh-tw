...
+
https://["瞭解如何在 Azure 中連線至 Linux VM"^]
...
+
link:reference-host-requirements-azure["檢閱 VM 需求"]。
...
+
此設定非常重要、因為託管身分識別可讓 Connector 虛擬機器在 Azure Active Directory 中識別自己、而無需提供任何認證。完成後、您需要指派您建立的自訂角色 https://["深入瞭解 Azure 資源的託管身分識別"^]。
