...
本頁說明如何直接從 Cloud Manager 在 GCP 中建立 Connector 。 link:concept-connectors.html#how-to-create-a-connector["瞭解部署Connector的其他方法"]。
...
在部署Connector之前、您必須確保GCP帳戶擁有正確的權限、而且已針對Connector VM設定服務帳戶。

.步驟
. 確認部署Connector的GCP使用者具有中的權限 https://occm-sample-policies.s3.amazonaws.com/Setup_As_Service_3.7.3_GCP.yaml["GCP 的連接器部署原則"^]。
+
...
+
.. https://cloud.google.com/iam/docs/creating-custom-roles#iam-custom-roles-create-gcloud["在 GCP 中建立角色"^] 這包括在中定義的權限 https://occm-sample-policies.s3.amazonaws.com/Policy_for_Cloud_Manager_3.9.10_GCP.yaml["GCP 的 Cloud Manager 原則"^]。同樣地、您需要使用 gCloud 命令列。
+
...
GCP使用者現在擁有建立Connector所需的權限、並已設定Connector VM的服務帳戶。
...
|===
| 服務帳戶 | 建立者 | 裝載於 | 服務專案權限 | 主機專案權限 | 目的 


| Cloud Manager服務帳戶 | 自訂 | 服務專案  a| 
* https://occm-sample-policies.s3.amazonaws.com/Policy_for_Cloud_Manager_3.9.10_GCP.yaml["在此.yaml檔案中找到的權限"^]
...
| Google API服務代理程式 | GCP | 服務專案  a| 
* （預設）編輯器

 a| 
* compute.networkUser

| 代表部署與GCP API互動。允許Cloud Manager使用共享網路。 


| Google Compute Engine預設服務帳戶 | GCP | 服務專案  a| 
* （預設）編輯器

 a| 
* compute.networkUser

| 代表部署來部署GCP執行個體和運算基礎架構。允許Cloud Manager使用共享網路。 
|===
附註：
...
.您需要的是 #8217 ；需要的是什麼
* 。 https://mysupport.netapp.com/site/info/cloud-manager-policies["必要權限"^] 如本頁第一節所述、適用於您的Google Cloud帳戶。
* Google Cloud 專案。
...
+
在我們的範例中、我們會使用已安裝gCloud SDK的本機Shell、但您可以在GCP主控台使用原生Google Cloud Shell。
