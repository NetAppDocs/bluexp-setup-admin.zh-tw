...
.步驟
. 確認部署Connector的Google Cloud使用者具有中的權限 https://occm-sample-policies.s3.amazonaws.com/Setup_As_Service_3.7.3_GCP.yaml["Google Cloud的Connector部署原則"^]。
+
...
+
.. https://cloud.google.com/iam/docs/creating-custom-roles#iam-custom-roles-create-gcloud["在 GCP 中建立角色"^] 這包括在中定義的權限 https://occm-sample-policies.s3.amazonaws.com/Policy_for_Cloud_Manager_3.9.10_GCP.yaml["適用於Google Cloud的Cloud Manager原則"^]。同樣地、您需要使用 gCloud 命令列。
+
...
* https://occm-sample-policies.s3.amazonaws.com/Setup_As_Service_3.7.3_GCP.yaml["在此.yaml檔案中找到的權限"^]
