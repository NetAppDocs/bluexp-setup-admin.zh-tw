...
.開始之前
設定 BlueXP  Connector 的人員必須使用不屬於以標準模式運作的 BlueXP  組織的登入登入來登入 BlueXP  。

如果您的 BlueXP  登入是與以標準模式運作的組織相關聯，則您需要註冊不屬於組織的新 BlueXP  登入或 NetApp 支援網站認證。否則，您將無法在設定畫面上看到啟用受限模式的選項。
