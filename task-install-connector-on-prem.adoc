...
.開始之前
* 查看有關的信息link:concept-connectors.html["連接器"] 。
...
在符合作業系統、RAM 和連接埠要求的主機上執行連接器軟體。安裝 Connector 之前、請先確定您的主機符合這些要求。
...
設定網路以確保連接器可以管理資源，並連接到目標網路並進行出站網路存取。
...
TIP: 為何不選擇 Google Cloud ？當 Connector 安裝在您的內部環境中時、就無法在 Google Cloud 中管理您的資源。您必須在 Google Cloud 中安裝連接器才能管理駐留在那裡的任何資源。
...
.結果
您現在應該擁有具有必要權限的 IAM 使用者存取金鑰。安裝連接器後，將這些憑證與BlueXP中的連接器關聯。
...
登入BlueXP並將連接器與您的組織關聯。登入方式取決於您使用BlueXP 的模式。如果您在標準模式下使用BlueXP ，則可以透過 SaaS 網站登入。如果您在受限或私有模式下使用BlueXP ，則可以透過連接器主機本機登入。
...
+
保持受限模式處於停用狀態，因為這些步驟在標準模式下使用BlueXP 。（此外、當 Connector 安裝在內部部署時、不支援受限模式。）
...
--
.開始之前
如果您剛剛建立這些 AWS 憑證，它們可能需要幾分鐘才能生效。請稍候幾分鐘、再將認證資料新增至BlueXP。
...
--
.開始之前
如果您剛剛建立了這些 Azure 憑證，它們可能需要幾分鐘才能可用。請稍候幾分鐘、再將認證資料新增至BlueXP。
