...
在部署連接器和Cloud Volumes ONTAP之前，您必須啟用多個 Google Cloud API。
...
.關於這項工作
建立 Connector 會使用預設組態、在 Google Cloud 中部署虛擬機器執行個體。建立後，請勿將連接器變更為具有較少 CPU 或 RAM 的較小 VM 執行個體。link:reference-connector-default-config.html["瞭解連接器的預設組態"]。
...
** *網路標籤*：如果使用透明代理，請向連接器實例新增網路標籤。網路標籤必須以小寫字母開頭，可以包含小寫字母、數字和連字號。標籤必須以小寫字母或數字結尾。例如，您可以使用“connector-proxy”標籤。
...
. 選取*「Add*」。
+
實例大約需要 7 分鐘才能準備就緒；請停留在頁面上直到程序完成。



.結果
過程完成後，即可從BlueXP使用連接器。
