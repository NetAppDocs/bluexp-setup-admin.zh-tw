...
. 識別主機上是否存在下列目錄：
+
/opp/application/NetApp/service-manager-2.

+
在2022年底之後建立的連接器應包含此目錄。

. 視目錄是否存在而定、執行命令來更新作業系統：
+
** 如果目錄存在、請以提高的權限執行下列命令：
+
[source, cli]
----
sudo -s
...
== 在沒有網際網路連線的位置升級Connector

如果您 link:task-install-connector-onprem-no-internet.html["已將Connector安裝在無法存取網際網路的位置"]，您可以在 NetApp 支援網站提供較新版本時升級 Connector。
...
. 從 Linux 主機執行解除安裝指令碼：
+
*/選項/應用程式/ NetApp /服務經理-2/uninstall.sh [無聲]*
