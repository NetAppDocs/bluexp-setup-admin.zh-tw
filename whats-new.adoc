...
image:screenshot-edit-name.png["螢幕擷取畫面、顯示在使用者設定下編輯您的姓名的能力。"]

如果您以同盟連線或 NetApp 支援網站 帳戶登入、則不支援編輯您的名稱。



=== Amazon EC2 執行個體中繼資料服務需求

您應該知道、在 AWS 中手動安裝 Connector 軟體之前、 EC2 執行個體需要 IMDSv1 。

link:reference-limitations.html["深入瞭解 AWS 中的 IMDSv1 需求"]。
