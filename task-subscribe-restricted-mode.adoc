...
summary: 從您的雲端供應商市場訂閱 NetApp 智慧服務，以小時費率或透過年度合約支付 BlueXP 服務費用。如果您向 NetApp 購買授權、您也需要訂閱市場方案。您的授權一律會先收費、但如果您超過授權容量或授權期限到期、則會以每小時費率收費。 
---
= 訂閱 NetApp 智慧服務（受限模式）
:hardbreaks:
:allow-uri-read: 
...
.開始之前
訂閱資料服務涉及將市場訂閱與與連接器關聯的雲端憑證關聯起來。如果您遵循「開始使用受限模式」工作流程、則您應該已經擁有 Connector 。若要深入瞭解link:task-quick-start-restricted-mode.html["以受限模式快速啟動 BlueXP"]，請檢視。
