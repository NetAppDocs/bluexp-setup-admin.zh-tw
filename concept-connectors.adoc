...
+
https://["深入瞭解內部ONTAP 部署的叢集探索與管理選項"^]
...
* Microsoft Azure
+
Azure 中的 Connector 應部署在與其管理的 Cloud Volumes ONTAP 系統所在的同一個 Azure 區域、或部署在中 https://["Azure區域配對"^] 適用於整個系統。Cloud Volumes ONTAP這項需求可確保Cloud Volumes ONTAP Azure Private Link連線可用於連接至相關的儲存帳戶。 https://["瞭Cloud Volumes ONTAP 解如何使用Azure Private Link"^]
...
. 探索現有的工作環境。
+
** https://["新增現有Cloud Volumes ONTAP 的元件系統至藍圖XP"^]
** https://["探索 ONTAP 叢集"^]
