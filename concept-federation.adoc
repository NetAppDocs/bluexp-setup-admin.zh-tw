...
如果您的身分識別供應商未出現在上方清單中、 https://["請遵循下列一般指示"^]
...
| 2. | BlueXP管理  a| 
前往 https://["NetApp聯盟設定頁面"^] 並建立與BlueXP的連線。
...
| 4. | BlueXP管理 | 從測試並啟用連線 https://["NetApp聯盟設定頁面"^]請注意、測試連線和啟用連線之間的頁面會重新整理。 
|===
...
在BlueXP管理員啟用連線之後、管理員可以隨時從更新連線 https://["NetApp聯盟設定頁面"^]
