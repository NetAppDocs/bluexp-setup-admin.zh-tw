...
[role="lead"]
對於Azure商業區域而言、最好直接從Cloud Manager建立Connector、但您也可以從Azure Marketplace（如果您願意）啟動Connector。對於Azure政府區域而言、您無法從Cloud Manager的SaaS網站將Connector部署到政府區域、因此下一個最佳選擇是從Azure Marketplace部署Connector。
...
如果Connector位於Azure商業區域、請開啟網頁瀏覽器並前往 https://cloudmanager.netapp.com[] 開始使用Connector with Cloud Manager。

如果Connector位於Azure政府區域、您可以開啟網頁瀏覽器並連線至Connector執行個體的IP位址、以使用Cloud Manager： http://_ipaddress_:80[]
