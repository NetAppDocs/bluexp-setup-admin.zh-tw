...
每種部署模式在出站連線需求、部署位置、安裝流程、驗證方法、可用資料和儲存服務以及收費方法方面有所不同。
...
--
受限模式:: 您在雲端（在政府區域、主權雲端區域或商業區域）安裝 BlueXP 連接器，並且與 BlueXP SaaS 層的出站連接有限。使用者可從 Connector 提供的網路型主控台、而非從 SaaS 層、在本機存取 BlueXP 。
+
--
...
--
私有模式:: 您在本機或雲端（安全區域、主權雲端區域或商業區域）安裝 BlueXP 連接器，且與 BlueXP SaaS 層沒有連線。使用者可從 Connector 提供的網路型主控台、而非從 SaaS 層、在本機存取 BlueXP 。
+
--
