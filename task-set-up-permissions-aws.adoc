...
summary: 設定 AWS 權限、讓您可以使用 BlueXP 來管理 AWS 中的資料和儲存基礎架構。設定權限的方式取決於您打算使用的安裝選項。 
---
= 設定 AWS 權限
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
設定 AWS 權限、讓您可以使用 BlueXP 來管理 AWS 中的資料和儲存基礎架構。設定權限的方式取決於您打算使用的安裝選項。
...
TIP: 當 BlueXP 建立 Connector 時、它會將一組新的權限套用至 Connector 執行個體、以啟用 Connector to AWS 資源。
