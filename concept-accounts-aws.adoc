...
從Cloud Manager部署Connector時、您需要為IAM使用者提供IAM角色或存取金鑰的ARN。您使用的驗證方法必須具有必要的權限、才能在AWS中部署Connector執行個體。所需權限列於 https://mysupport.netapp.com/site/info/cloud-manager-policies["AWS 的連接器部署原則"^]。
